...
if (isset($_POST["submit"])) {
   ...
   if (empty($_POST["name"]) || empty($_POST["price"]) || empty($_POST["ImagePath"])) {
      header("Location: add-new.php?msg=Please fill in all fields");
      return;
   }

   if (!is_numeric($_POST["price"])) {
      header("Location: add-new.php?msg=Price must be a number");
      return;
   }

   if (!preg_match("/^[a-zA-Z\s]+$/", $_POST["name"])) {
      header("Location: add-new.php?msg=Name must only contain letters and spaces");
      return;
   }
   ...
   if (mysqli_num_rows($result) > 0) {
      header("Location: add-new.php?msg=Name already exists");
      return;
   }

   $sql = "INSERT INTO `products`(`id`, `name`, `price`, `ImagePath`) VALUES (NULL,'$name','$price','$ImagePath')";
